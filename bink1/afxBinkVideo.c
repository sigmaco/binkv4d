...
DLLEXPORT afxError CreateBinkTextures(afxBinkVideo *bnk)
{
    ...
    afxDrawContext dctx;
    AfxGetDrawInputContext(bnk->din, &dctx);
    ...
    {
        if (AfxAcquireBuffers(dctx, 1, &bufi, &bnk->stageBuffers[i])) AfxThrowError();
        else
        {
            if (AfxAcquireRasters(dctx, rasCnt, texi, bnk->rasters[i])) AfxThrowError();
            else
            {
                afxByte* start;
                AfxMapBuffer(bnk->stageBuffers[i], 0, bnk->stageBufSiz, NIL, (void**)&start);
...
void LockBinkTextures(afxBinkVideo *bnk)
{
    ...
    for (afxInt i = 0; i < bnk->buffers.TotalFrames; ++i)
    {
        ...
        if (AfxMapBuffer(bnk->stageBuffers[i], 0, bnk->stageBufSiz, NIL, (void**)&start))
        {
            AfxThrowError();
            break;
        }
...
DLLEXPORT afxError AfxBinkBlitFrame(afxBinkVideo *bnk, avxCmdb cmdb)
{
    ...
    AfxAssertObjects(1, &bnk->yv12ToRgbaDtec, afxFcc_DTEC);
    AvxCmdApplyDrawTechnique(cmdb, bnk->yv12ToRgbaDtec, 0, NIL, NIL);
...
DLLEXPORT afxError AfxOpenVideoBink(afxBinkVideo *bnk, afxUri const *uri)
{
    ...
    afxFile file;
    AfxOpen(uri, afxFileFlag_R, &file);
    AfxGetResolvedFileUri(file, &uri2.uri, &uri2.uri);
...
DLLEXPORT afxError AfxDropVideoBink(afxBinkVideo *bnk)
{
    ...
    if (bnk->yv12ToRgbaDtec)
        AfxReleaseObjects(1, &bnk->yv12ToRgbaDtec);

    //AfxReleaseObject(&bnk);
    return err;
}

DLLEXPORT afxError AfxSetUpBinkPlayer(afxBinkVideo *bnk, afxDrawInput din)
{
    afxError err = AFX_ERR_NONE;
    bnk->running = FALSE;
    ...
    AfxAcquireSamplers(dctx, 1, &smpSpec, &bnk->samplers[0]);
    ...
    AfxLoadDrawTechniques(din, 1, &uri, &bnk->yv12ToRgbaDtec);
    AfxAssert(bnk->yv12ToRgbaDtec);
